...
assert.match(styles, /\.ask-ai-panel\.expanded[\s\S]*--ask-ai-workspace-inset: 16px;[\s\S]*--ask-ai-workspace-sidebar-width: clamp\(232px, 18vw, 260px\);[\s\S]*--ask-ai-workspace-header-height: 96px;[\s\S]*inset: var\(--ask-ai-workspace-inset\);[\s\S]*width: auto;/, "Ask AI expanded mode should fill the viewport with a compact chat history sidebar and shared header height.");
